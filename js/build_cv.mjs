...
console.log('%cHola!', "color: #d8aa2c;");
console.log('Bienvenido a mi CV :)');
console.log('Si estás viendo esto, te puede interesar información más técnica. Te invito a ver mis repositorios en;');
console.log('https://github.com/imperioame');
...
function build(lenguaje_solicitado) {
    ...
    let lenguaje = lenguaje_solicitado;
    document.documentElement.setAttribute("lang", lenguaje);
    ...
    */
    ...
    if (domicilio) {
        ...
        p.innerHTML = `${lenguaje == 'es' ? 'Domicilio' : 'Address'} ${domicilio}, ${barrio}<br>CP: ${codigo_postal}`;
        document.getElementById('detalles').appendChild(p);
    }
    if (fecha_nacimiento) {
        let p = document.createElement('p');
        p.innerHTML = `${lenguaje == 'es' ? 'Fecha de Nacimiento:' : 'Born in:'} ${fecha_nacimiento}`;
        document.getElementById('detalles').appendChild(p);
    }
    ...
    for (let i = 0; i < trabajos_lenguaje_actual.length && i <= CANTIDAD_MAX_ELEMENTOS; i++) {
        ...
        h4.innerHTML += ` ${lenguaje == 'es' ? 'en' : 'in'} ${trabajos_lenguaje_actual[i].getEmpleador()}`;
        ...
        if (trabajos_lenguaje_actual[i].getWeb()) {
            ...
            a.innerHTML = `${lenguaje == 'es' ? 'Ver:' : 'See:'} ${trabajos_lenguaje_actual[i].getWeb()}`;
            p.appendChild(a);
        }
    ...
    if (cvs_lenguaje_actual.length) {
        ...
        let p = document.createElement('p');
        p.innerHTML = (lenguaje == 'es' ? 'Descargar' : 'Download') + '<br>pdf';
        a.appendChild(p);
